...
from utils import init_encoded_traits, server_to_region, CURRENT_VERSION, update_item_augment_map, MY_SERVER, MY_REGION, \
    get_trait_name, item_in_class, init_request_session
...
def main():
    ...
    server_to_matches(cur, session, server, region)
...
def insert_match(cursor, session, server, region, match_id, retried=False):
    ...
    if res.status_code != 200:
        ...
        if res.status_code == 429:
            if not retried:
                time.sleep(25)
                insert_match(cursor, session, server, region, match_id, True)
        return
    result = res.json()
    info = result['info']
                ...
                if trait['tier_current'] > 0:
                    ...
                    if len(notable_traits) < 3 and api_name in TRAIT_TO_ENCODED:
                        if trait['tier_current'] >= 3:
                            ...
                            if trait['tier_current'] != 1 or trait['tier_total'] != 1:
...
def summoner_to_matches(cursor, session, server, region, summonerId, count=10, retried=False) -> list:
    if summonerId == '' or summonerId is None:
        return []
    '''return list_of_matches'''
    puuid = name_to_puuid(server, session, server_to_region(server), summonerId)
    ...
    if res.status_code != 200:
        ...
        if res.status_code == 429:
            if not retried:
                time.sleep(25)
                return summoner_to_matches(cursor, session, server, region, summonerId, count, True)
        else:
            return []
...
def server_to_matches(cursor, session, server, region):
    '''
    ...
    challengers = get_league(cursor, session, server, region, 'challenger')
    grandmasters = get_league(cursor, session, server, region, 'grandmaster')
    # mastercnt = 2000 - len(challengers) - len(grandmasters)
    # masters = get_league(cursor, session, server, region, 'master', 200)
    for i, summonerId in enumerate(challengers):
        # print(f"player {i}")
        list_of_matches = summoner_to_matches(cursor, session, server, region, summonerId)
        if list_of_matches:
            for match in list_of_matches:
                if match:
                    insert_match(cursor, session, server, region, match)
    for i, summonerId in enumerate(grandmasters):
        # print(f"player {i}")
        list_of_matches = summoner_to_matches(cursor, session, server, region, summonerId)
        if list_of_matches:
            for match in list_of_matches:
                if match:
                    insert_match(cursor, session, server, region, match)
    # for i, summonerId in enumerate(masters):
    #     print(f"player {i}")
    #     list_of_matches = summoner_to_matches(cursor, session, server, region, summonerId, count=20)
    #     if list_of_matches:
    #         for match in list_of_matches:
    # if match:
...
def get_league(cursor, session, server, region, league, mastercnt=0, retried=False) -> list:
    ...
    if league in ['challenger', 'grandmaster', 'master']:
        ...
        res = session.get(high_url, headers=headers)
        if res.status_code != 200:
            print(res.status_code)
            if res.status_code == 429:
                if not retried:
                    time.sleep(25)
                    return get_league(cursor, session, server, region, league, mastercnt, True)
            else:
        ...
        if league == 'master':
            ...
            summonerIds = [x['summonerId'] for x in json['entries'] if x['leaguePoints'] > 0]
            if len(summonerIds) <= mastercnt:
                return summonerIds
            return random.sample(summonerIds, mastercnt)
        print(f'{league}s count in {server}: {len(json["entries"])}')
        return [x['summonerId'] for x in json['entries']]
    else:
        league = league.upper()
        league_cnt = 0
        for division in ['I', 'II', 'III', 'IV']:
            ...
            while page:
                ...
                res = session.get(low_url, headers=headers)
                if res.status_code != 200:
                    print(res.status_code)
                    if res.status_code == 429:
                        time.sleep(30)
...
def name_to_puuid(server, session, region, summonerId, retried=False) -> str:
    if not summonerId:
        return ''
    url = f"https://{server}.api.riotgames.com/tft/summoner/v1/summoners/{summonerId}"
    headers = {'X-Riot-Token': RIOT_API}
    res = session.get(url, headers=headers)
    if res.status_code != 200:
        print(res.status_code)
        if res.status_code == 429:
            if not retried:
                time.sleep(25)
                return name_to_puuid(server, session, region, summonerId, True)
        return ''
    json = res.json()
    return json['puuid']
